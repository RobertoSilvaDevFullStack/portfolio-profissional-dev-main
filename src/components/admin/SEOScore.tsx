...
const SEOScore = ({ title, description, slug, content }: SEOScoreProps) => {
  const analyzeSEO = (): SEOAnalysis => {
    ...
    if (!title) {
      ...
      issues.push({
        type: "warning",
        message: "Título muito curto (mínimo recomendado: 30 caracteres)",
      });
      score -= 10;
    ...
    if (!description) {
      ...
      issues.push({
        type: "warning",
        message:
          "Meta descrição muito curta (mínimo recomendado: 120 caracteres)",
      });
      score -= 10;
      ...
      issues.push({
        type: "warning",
        message:
          "Meta descrição muito longa (máximo recomendado: 160 caracteres)",
      });
      score -= 10;
    } else {
      issues.push({
        type: "success",
        message: "Meta descrição com tamanho ideal",
      });
    }
    ...
    if (!slug) {
      ...
      issues.push({
        type: "warning",
        message: "Slug contém caracteres especiais ou maiúsculas",
      });
      score -= 5;
    } else {
    ...
    if (!content || content.length < 300) {
      issues.push({
        type: "warning",
        message: "Conteúdo muito curto (mínimo recomendado: 300 caracteres)",
      });
      score -= 10;
    ...
    const commonWords = titleWords.filter(
      (word) => descriptionWords.includes(word) && word.length > 3
    );

    if (commonWords.length > 0) {
      issues.push({
        type: "success",
        message: "Palavras-chave consistentes entre título e descrição",
      });
    } else {
      issues.push({
        type: "warning",
        message: "Considere usar palavras-chave do título na descrição",
      });
      score -= 5;
    }
